...
class ImgAttentionEncoder(nn.Module):
  ...
  def __init__(self, embed_size):
    ...
    self.fc = nn.Sequential(nn.Linear(self.cnn[-3].out_channels, embed_size), nn.Tanh())     # feature vector of image
...
class SANModel(nn.Module):
  def __init__(self, embed_size, qst_vocab_size, ans_vocab_size, word_embed_size, num_layers, hidden_size):
    ...
    self.tanh = nn.Tanh()
    self.mlp = nn.Sequential(nn.Dropout(p=0.5), nn.Linear(embed_size, ans_vocab_size))
